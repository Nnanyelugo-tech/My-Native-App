...
export function ConfirmDeleteModal({ title, onConfirm }: Props) {
      ...
      <View className="bg-surface-card rounded-3xl p-6 items-center shadow-lg border border-border-subtle">
        ...
        <View className="flex-row gap-3 w-full">
          ...
          <Pressable
            ...
            disabled={isLoading}
            className="flex-1 py-3 rounded-2xl bg-danger-main items-center justify-center"
          >
            {isLoading ? (
              <AnimatedSpinner size="small" color={colors.textInverse} />
            ) : (
              <AppText className="font-bold text-[12px] text-text-inverse">Yes, Delete</AppText>
            )}
          </Pressable>
        </View>
      </View>
